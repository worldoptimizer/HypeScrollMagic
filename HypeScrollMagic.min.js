...
offset:t,duration:r,triggerHook:y});c.pin&&e.setPin(d,{pushFollowers:!1});if(c.reset)e.on("add",function(k){p.pauseTimelineNamed(a);p.goToTimeInTimelineNamed(0,a)});e.on("progress",function(k){var m=p.durationForTimelineNamed(a);0!==m&&p.goToTimeInTimelineNamed(k.progress*m,a)});if(f.behavior.enter)e.on("enter",function(k){l("Enter",k)});if(f.behavior.leave)e.on("leave",function(k){l("Leave",k)});e.addTo(h[g]);e.addIndicators&&(f.addIndicators||c.addIndicators)&&e.addIndicators({name:a,colorStart:c.indicatorColor,
colorEnd:c.indicatorColor,colorTrigger:c.indicatorColor});q.push(e);return e}if(window.ScrollMagic){ScrollMagic.Scene.prototype.addIndicators=null;ScrollMagic.Scene.prototype.removeIndicators=null;var h={},q=[],f={options:{triggerHook:.5,pin:!1,reset:!0,indicatorColor:"grey"},behavior:{enter:!0,leave:!0},timelineName:"Main Timeline",logBehavior:!1,addIndicators:!0};!1==="HYPE_eventListeners"in window&&(window.HYPE_eventListeners=[]);window.HYPE_eventListeners.push({type:"HypeDocumentLoad",callback:function(b,
...
(l.addIndicators=!0);c.forEach(function(g){u(b,a,g,l)})})}});window.HYPE_eventListeners.push({type:"HypeSceneUnload",callback:function(b,d){var a=d.id;q.forEach(function(c){c.destroy(!0)});q.length=0;h[a+"_v"]&&(h[a+"_v"].destroy(!0),delete h[a+"_v"]);h[a+"_h"]&&(h[a+"_h"].destroy(!0),delete h[a+"_h"])}});return{version:"1.0.5",setDefault:function(b,d){"object"==typeof b?f=b:f[b]=d},getDefault:function(b){return b?f[b]:f}}}console.error("ScrollMagic is not available.")}());
